...
0000000000400da0 <main>:
  ...
  400e19:	e8 84 05 00 00       	call   4013a2 <initialize_bomb>  #调用initialize_bomb
  400e1e:	bf 38 23 40 00       	mov    $0x402338,%edi
  400e23:	e8 e8 fc ff ff       	call   400b10 <puts@plt>
  400e28:	bf 78 23 40 00       	mov    $0x402378,%edi
  400e2d:	e8 de fc ff ff       	call   400b10 <puts@plt>
  400e32:	e8 67 06 00 00       	call   40149e <read_line>   #调用read_line读取输入
  400e37:	48 89 c7             	mov    %rax,%rdi   #将输入的内容存入%rdi
...
0000000000400ee0 <phase_1>:
  ...
  400ef0:	74 05                	je     400ef7 <phase_1+0x17>  #调用strings_not_equal函数判断输入的字符串是否等于"Public speaking is very easy."，如果不相等则调用explode_bomb函数
...
0000000000400efc <phase_2>:
  ...
  400f02:	48 89 e6             	mov    %rsp,%rsi #%rsi是第二个参数，指向栈顶，%rdi是我们传入的第一个字符串
  400f05:	e8 52 05 00 00       	call   40145c <read_six_numbers> #第一个参数是我们传入的字符串指针，第二个参数是栈顶指针
...
0000000000401338 <strings_not_equal>:
  ...
  40133a:	55                   	push   %rbp
  40133b:	53                   	push   %rbx
  40133c:	48 89 fb             	mov    %rdi,%rbx #第一个参数移动到%rbx
  40133f:	48 89 f5             	mov    %rsi,%rbp #第二个参数移动到%rbp
  401342:	e8 d4 ff ff ff       	call   40131b <string_length> #调用string_length函数
  401347:	41 89 c4             	mov    %eax,%r12d #将返回值存储到%r12d（第一个字符串的长度）
  40134a:	48 89 ef             	mov    %rbp,%rdi #将第二个字符串的值移动到%rdi
  40134d:	e8 c9 ff ff ff       	call   40131b <string_length>
  401352:	ba 01 00 00 00       	mov    $0x1,%edx
  401357:	41 39 c4             	cmp    %eax,%r12d #比较两个字符串的长度
  40135a:	75 3f                	jne    40139b <strings_not_equal+0x63>
  40135c:	0f b6 03             	movzbl (%rbx),%eax #将第一个字符串的第一个字符移动到%eax
  40135f:	84 c0                	test   %al,%al
  401361:	74 25                	je     401388 <strings_not_equal+0x50> #如果第一个字符串的第一个字符为NULL(即为空字符串)，则跳转到401388
  401363:	3a 45 00             	cmp    0x0(%rbp),%al #比较第一个字符串的第一个字符和第二个字符串的第一个字符
  ...
  40137f:	75 e9                	jne    40136a <strings_not_equal+0x32> #结果非零就进行跳转回循环
...
000000000040145c <read_six_numbers>:
  ...
  401460:	48 89 f2             	mov    %rsi,%rdx #栈顶指针移动到%rdx
  401463:	48 8d 4e 04          	lea    0x4(%rsi),%rcx #栈顶指针一个字节的偏移量移动到%rcx
  401467:	48 8d 46 14          	lea    0x14(%rsi),%rax #栈顶指针14位的偏移量移动到%rax
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp) #将%rax的值存储到栈顶指针偏移8位的位置
